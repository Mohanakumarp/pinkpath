...
// Web uses localhost by default. Mobile reads the backend URL from Expo env.
...
export const sendMessageToBot = async (message) => {
  try {
    ...
    if (error?.name === 'AbortError') {
      return "The request timed out before the server answered. On web, verify the localhost backend. On phone, verify EXPO_PUBLIC_BACKEND_URL in your Expo env.";
    }

    return "I'm having trouble connecting to the backend. On web use localhost. On phone set EXPO_PUBLIC_BACKEND_URL in your Expo env.";
  }
};